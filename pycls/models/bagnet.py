...
class BagNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.relu = nn.ReLU(inplace=True)
        layers, strides, kernel3 = cfg.BAGNET.LAYERS, cfg.BAGNET.STRIDES, cfg.BAGNET.KERNEL3
        ...
        self.fc = nn.Linear(512 * Bottleneck.expansion, cfg.MODEL.NUM_CLASSES)
        self.avg_pool = True
...
class ClippedBagNet(BagNet):
    ...
    def __init__(self):
        super().__init__()
        self.avg_pool = False
        self.a = cfg.BAGNET.CLIP_A
        self.b = cfg.BAGNET.CLIP_B

    def forward(self, x):
        x = super().forward(x)  # batch_size x patches x patches x num_classes
        x = torch.tanh(self.a * x + self.b)
